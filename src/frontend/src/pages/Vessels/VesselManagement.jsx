...
import { Search, Filter, Plus, MoreHorizontal, X, Ship } from 'lucide-react';
...
// ── Add Vessel Modal ──────────────────────────────────────────────────────────
function AddVesselModal({ onClose, onSuccess }) {
      ...
      <div className="bg-surface border border-outline-variant rounded-xl shadow-2xl w-full max-w-lg mx-4 animate-in fade-in zoom-in-95">
        ...
        <div className="flex items-center justify-between p-6 border-b border-outline-variant">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-semibold text-on-surface">Add New Vessel</h2>
              <p className="text-xs text-on-surface-variant">Register a vessel in the system</p>
            </div>
          </div>
          <button
            onClick={onClose}
...
export function VesselManagement() {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const vessels = Array.isArray(response)
        ? response
        : response?.items ?? [];
      setItems(vessels);
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => (
        <span className="text-sm text-on-surface-variant">{row.vessel_type || '—'}</span>
      ),
    },
    {
      header: 'Length (m)',
